...
public class Sort {
    ...
    //Bucket sort
    public int[] bucketSort(int[] arr, int maxVal) {
        Bucket[] buckets = new Bucket[10];
        for (int index = 0; index < buckets.length; index++) {
            buckets[index] = new Bucket();
        }
        for (int n : arr) {
            int bIndex = n * 10 / maxVal;
            buckets[bIndex].bucket.add(n);
        }
        int index = 0;
        for (Bucket b : buckets) {
            int[] bArray = new int[b.bucket.size()];
            for (int i = 0; i < bArray.length; i++)
                bArray[i] = b.bucket.get(i).intValue();
            bArray = insertSort(bArray);
            for (int num : bArray) {
                arr[index++] = num;
            }
        }
